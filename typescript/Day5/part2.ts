...
for (var i = 0; i < seeds.length; i = i+2) {
    let currSeedNum = seeds[i]
    let entries = seeds[i+1]
    for (var j = 0; j <= entries; j++) {
        let seed = currSeedNum+j
        let currLocation = getLocationOfSeed(seed)
        if (seedLocation == 0) {
            seedLocation = currLocation
        }
        if (currLocation < seedLocation) {
            seedLocation = currLocation
        }
    }
...
console.log("Smallest location", seedLocation)
